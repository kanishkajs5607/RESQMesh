...
Status: requires physical Android phones; not executed in the coding environment.

## Before the demo

1. Build/install the SAME debug APK on three Android 8+ phones with Google Play services.
...
4. A: Create SOS → Fill hackathon example. This sets Collapse, CRITICAL, 3 people, 1 injured and a clearly labelled DEMO location. Replace with device location if available.
5. A: Save & Share SOS.
...
8. On B, pause mesh, fully close the app, reopen it. Verify the SAME packet remains saved. This demonstrates persistent storage.
...
10. The saved SOS forwards automatically after connection. C must show:
    - CRITICAL SOS / 3 people / 1 injured;
    - Trapped after building collapse;
    - the original coordinates and timestamp;
    - the SAME message ID and origin A;
    - hop count 2 and path A → B → C.
11. Disconnect and reconnect B and C. The inbox must still contain one copy of this packet.
...
B has now stored the emergency. We stop A and restart B to prove persistence.
